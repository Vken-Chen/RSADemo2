...
+ (SecKeyRef)privateKeyFromPem:(NSString *)privateKey error:(NSError **)error
{
    ...
    if(keyData!=nil)
    {
        return [self rsaKeyFromData:keyData andTag:@"vkrsa.privatekey" error:error];
    }else{
        //todo error handle
        return nil;
    }
...
+ (SecKeyRef)publicKeyFromPem:(NSString *)publicKey error:(NSError **)error
{
    ...
    if(keyData!=nil)
    {
        return [self rsaKeyFromData:keyData andTag:@"vkrsa.publickey" error:error];
    }else{
        //todo error handle
        return nil;
    }
...
+ (SecKeyRef)publickeyFromDer:(NSData*)data error:(NSError **)error
{
    if(data == nil)
    {
        *error = [NSError rsaErrorWithDescription:@"privateKeyFromDer der data is nil"];
        return nil;
    }
...
+( SecKeyRef)rsaKeyFromData:(NSData*)data andTag:(NSString*)tag error:(NSError **)error
{
