...
- (IBAction)onEncrypt:(id)sender {
    ...
    tickStart = clock();
    SecKeyRef publicKey2 = [VKRSAKey publicKeyFromPem:_publicKeyPem error:&error];
    clock_t time2 = clock()-tickStart;
    NSLog(@"改进PEM方式的耗时 %lu",time2);
    
    
    NSString* derFile = [[NSBundle mainBundle]pathForResource:@"rsa_public_key" ofType:@"der"];
    NSData* derData = [NSData dataWithContentsOfFile:derFile];
    tickStart = clock();
    SecKeyRef publicKey3 = [VKRSAKey publickeyFromDer:derData error:&error];
    clock_t time3 = clock()-tickStart;
    NSLog(@"der方式的耗时 %lu",time3);
    
    
    _encryptData = [VKRSA encryptData:originData withKey:publicKey3];
}
